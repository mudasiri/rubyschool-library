...
describe Book do
  describe '#initialize' do
    before :each do
      @book = Book.new('Oliver Twist', 'Shake Spare')
    end
    ...
    it { expect(@book).to have_attributes(title: 'Oliver Twist') }
    it { expect(@book).to have_attributes(author: 'Shake Spare') }
  end

  describe '#to_json' do
    it 'returns a JSON representation of the Book object' do
      @book = Book.new('The gods are not to blame', 'Ola Rotimi')
      expect(@book.to_json).to eq({ 'title' => 'The gods are not to blame', 'author' => 'Ola Rotimi', 'rentals' => [] })
    end
  end
end
...
describe BookUserInterface do
  ...
  describe '#create_book' do
    it 'creates a new book and saves it to a file' do
      ...
      allow(book_user_interface).to receive(:handle_title_input).and_return('Enjoy Your Life')
      allow(book_user_interface).to receive(:handle_author_input).and_return('Abdul Rahman Al Arifi')

      book_user_interface.create_book
      expect(book_user_interface.books.count).to eq 1
      expect(book_user_interface.books.first['title']).to eq 'Enjoy Your Life'
      expect(book_user_interface.books.first['author']).to eq 'Abdul Rahman Al Arifi'
    end
  end
